...
public partial class TakeActivityPage : ContentPage
{
    ...
    private void ShowFeedback(SubmitFeedbackResponse feedback)
    {
        ...
        SubmitButton.BackgroundColor = Color.FromArgb("#5BBF8A");
        SubmitButton.IsEnabled = true;
        SubmitButton.Clicked -= OnSubmit;
        SubmitButton.Clicked += async (s, e) => await Navigation.PopAsync();

        // Banner de calificación automática (HU#10)
        ...
        if (feedback.Total > 0)
        {
            ...
            ScoreEmojiLabel.Text = passed ? "🎉" : "😔";
            ScoreLabel.Text = $"{score}% — {(passed ? "¡Aprobado!" : "No aprobado")}";
            ScoreLabel.TextColor = passed ? Color.FromArgb("#2E8A5E") : Color.FromArgb("#C0392B");
            ScoreSubLabel.Text = $"{feedback.Correct} de {feedback.Total} respuestas correctas";
            ScoreAccent.BackgroundColor = passed ? Color.FromArgb("#5BBF8A") : Color.FromArgb("#F4A0C0");
        }
        else
        {
            // Solo preguntas abiertas, sin calificación automática
            ScoreEmojiLabel.Text = "✅";
            ScoreLabel.Text = "Actividad enviada";
            ScoreLabel.TextColor = Color.FromArgb("#2E8A5E");
            ScoreSubLabel.Text = "Tu profesor revisará las respuestas";
            ScoreAccent.BackgroundColor = Color.FromArgb("#5BBF8A");
        }
...
internal class QuestionAnswerWidget
{
    ...
    public void ShowFeedback(QuestionFeedback? fb)
    {
        ...
        // Result summary
        var summaryColor = fb.IsCorrect ? Color.FromArgb("#C8F0DC") : Color.FromArgb("#F4A0C0");
        _layout.Children.Add(new Label
        {
